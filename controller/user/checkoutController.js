...
const getCheckout = async (req, res) => {
  try {
    ...
    const coupons = await couponModel
      .find({ status: true, expiryDate: { $gt: new Date() } })
      ...
      try {
        const userId = req.session.user
          ...
          : req.user
            ? req.user._id
            : null;
        const checkoutData = await getCheckoutPageDataService(
          userId,
          req.query,
        );
        ...
        console.error(
          "Error rendering checkout page on validation failure",
          renderError,
        );
  ...
  try {
    ...
    if (error.statusCode === 400 && error.errors) {
      try {
        ...
        console.error(
          "Error rendering checkout page on add validation failure",
          renderError,
        );
        return res.status(500).send("Internal Server Error");
      }
