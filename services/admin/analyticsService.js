...
export const getAnalyticsStatsService = async (period = 'all') => {
    ...
    const allOrdersTotal = allOrdersTotalResult.length > 0 ? allOrdersTotalResult[0].total : 0;
    ...
    try {
        categorySales = await Order.aggregate([
            ...
            { $unwind: '$categoryInfo' },
            { $group: { _id: '$categoryInfo.categoryName', sales: { $sum: '$items.subtotal' } } },
            { $sort: { sales: -1 } }
        ]);
    } catch (err) {
    ...
    if (categorySales && categorySales.length > 0) {
        const totalSalesSum = categorySales.reduce((acc, curr) => acc + curr.sales, 0) || 1;
    ...
    try {
        topProducts = await Order.aggregate([
            ...
            {
                $project: {
                    name: 1, image: 1, sales: 1, quantitySold: 1,
    ...
    const defaultProducts = [
        { name: 'Ultrawide 49" OLED', categoryName: 'Monitors', sales: 240200, growth: '+8%', image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCuBeCA4vyN1nSz41OBT1nhwsRVEWETXRpoBFHQpfVq7U3cJ7S_pmh5GPKxS3QBhWHqNoT3kNl4cV_to00wd-05MpWtcnPihXkZz2rasH4et_BXu9nbZrqTRuJBxuefLuuCCm0btPfYmrswxjw5ru8RgBoA1_cZilgPkREbSgPeaFIc55uK0aPitqgkYfKY5LIYLwgohV84v_VuxOt1hfoZH2C1Lg8IIl93HA5alKtAPlRQOjTs1KK1Jwmf9_earAneTm2OibL3Bi1j' },
        { name: 'Apex Pro TKL', categoryName: 'Keyboards', sales: 182500, growth: '+12%', image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAG71tbXljNXKoqa-SjUqYhvJa8rOm-HsCkVS0rCmO1fAu-tFJlAMLKe8IfSkZw1Z09zWMTTPUMQKKLfasFDBCXS7hiT3x7F06wVRLxkGy9dgtR_rEUhKj30Z6U_65ybmpGx9EDFw3Sgim9am-0ahN2d7scJmzTAf0qcSFBUx-0OflrcnJDGDxh4NAqY9usoAQhY8GVASr_QR4_ugl8ouRN5yYA1ZWx8lbeyUSw3q8ZcQWZKVHO3goIIIVJDW5wKC9UQyoodk8-THjL' },
        { name: 'Elite Pro Wireless', categoryName: 'Headsets', sales: 156900, growth: '+5%', image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDb7wGlF5a-IpALx56n3UFS9ZGKB8-ysY2FGRIhgtAhyQN593dhjI3Z_-DnGRv0oyUopTRFTyLtT03yQ40QxqtEdJMS4vZbaLf0ZbbdPOhFgMsHYH0Khta0LortjMlcRUK8nac_py2ByYpKr__VKuzr2NtmraGxRN55fCtBNntkEOMXLB9jMeDMEeI9F9o8M85O9yD87SWPfS4mCWXYCqSVE2LFDBXWIt656x6F7govR9Zbvhs4asyFb0Gt4x7Kkm7MzwCydDnPiGBx' },
        { name: 'G Pro Superlight', categoryName: 'Mice', sales: 110400, growth: '+21%', image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCEBvZ6SFkGOU-JXrtyE6Dew_3C3XbACqFW1CmB4uCtuhuNDsHc3TLAL2nqdi4Sfj4YtbCkXZSGvWN3AamfiUrn1b04SRH5cxWMild8hpclg0YVR5KyN22E3nEB8ffnfJGTooCOiNssEWl9-wzB5HNWtCFlGX8MZS4fsftvCMqzGa-RlSXyKf7CEiD2K5sxACilf2B3I61Ngs1chqFC_rYIvtZRwVeXtvEiG3WvP4OmSH4iiVIYHBf4HYNF0Lt90zC9w9orqwITfEsm' },
        { name: 'MX Studio V2', categoryName: 'Headsets', sales: 94200, growth: '+15%', image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAwA9tJjtjtw9-Fm1MZo8J1KdFJksTLfBkNANy-gGtc-oPSRn9wJK0Xni-krwzARFhPXbD4zYHwGQ9PIUcUIcYU1L6tfz-tcvImmLlfhhAAEQzO6EHcHtkWnPL7b0dRm-K2l6Z4zrJk70HaJOFQHFVZhzjgt8va22_LklL30c27dN9yABYnV3frorLQuLWR8xY59EvBI2ISaBDglyZH1-Bdvo2jqkAFU_AuwE3uh8yiAipSbqefn2cOqETdJh5CYbyPMpiikWtguCLt' }
    ];
    ...
    if (period === 'day') {
        ...
        trendLabels = ['12 AM', '3 AM', '6 AM', '9 AM', '12 PM', '3 PM', '6 PM', '9 PM'];
        ...
        allOrders.forEach(order => {
            const orderDate = new Date(order.createdAt);
            const diffMonths = (orderDate.getFullYear() - allStart.getFullYear()) * 12 + (orderDate.getMonth() - allStart.getMonth());
