...
const getCouponsService = async () => {
    ...
    const allCoupons = await couponModel.find({}).populate('applicableCategories').sort({ createdAt: -1 });
    const categories = await categoryModel.find({ status: true, isHidden: false }).sort({ categoryName: 1 });
...
const updateCouponService = async (id, couponData) => {
    ...
    if (isExpired) {
        status = false;
    } else if (couponToUpdate.expiryDate && new Date(couponToUpdate.expiryDate) <= today) {
