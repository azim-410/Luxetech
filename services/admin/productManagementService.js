...
const addProductService = async (productData, body, files) => {
  ...
  if (!productData.name || productData.name.trim() === "") {
    ...
    if (nameTrimmed.length < 3) {
      ...
      errors.productName =
        "Product name must contain only alphanumeric characters, spaces, hyphens, quotes, or parentheses";
    }
  }
...
const updateProductService = async (id, body, files) => {
  ...
  // ─── Variant Validation ─────────────────────────────
  const removedImagesList = body.removedImages
    ? body.removedImages
        .split(",")
        .map((img) => img.trim())
        .filter(Boolean)
  ...
  if (variantData.length === 0) {
    ...
    for (const v of variantData) {
      ...
      if (v._id) {
        ...
        if (dbVar) {
          const retainedImages = (dbVar.images || []).filter(
            (img) => !removedImagesList.includes(img),
          );
  ...
  const formIds = variantData.filter((v) => v._id).map((v) => v._id.toString());
