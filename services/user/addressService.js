...
const validateAddressData = ({
  ...
  if (!zipCode || zipCode.trim() === "") {
    ...
    errors.zipCode =
      "Zip code must be 4–10 alphanumeric characters (e.g. 110001).";
  }
  ...
  if (!phoneNumber || phoneNumber.trim() === "") {
    ...
    errors.phoneNumber =
      "Phone number cannot consist of the same digit repeated.";
  }
...
const editAddressService = async (addressId, addressData) => {
  ...
  const inputDefault =
    isDefault === "true" || isDefault === "on" || isDefault === true;
  const isSame =
  ...
  if (isSame) {
    errors.global =
      "No changes made. Please modify at least one field to save updates.";
  }
