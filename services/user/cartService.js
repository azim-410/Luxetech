    ...
    try {
        ...
        if (cart && cart.items) {
            for (let i = 0; i < cart.items.length; i++) {
                ...
                const isProductUnavailable = !product || product.isDeleted || product.isHidden;
                const isVariantUnavailable = variant ? (variant.status === false || variant.isDeleted === true) : false;
...
const updateCartQuantityService = async (userId, productId, variantId, newQuantityInput) => {
    try {
        console.log('--- updateCartQuantityService called ---');
        console.log('productId:', productId, 'variantId:', variantId, 'newQuantityInput:', newQuantityInput);
        ...
        console.log('Found cart.items:', cart.items.map(item => ({ productId: item.productId, variantId: item.variantId, quantity: item.quantity })));
...
const addAllFromWishlistToCartService = async (userId) => {
    try {
        ...
        const activeWishlistItems = wishlist.products.filter(item => item.isActive !== false);
        ...
        for (const item of activeWishlistItems) {
            ...
            if (product && !variant) {
                variant = await variantModel.findOne({ productId: product._id, status: true });
            }
            ...
            if (isUnavailable) {
                return { success: false, message: 'Cannot add items. One or more products in your wishlist are unavailable.' };
            }

            const stock = variant ? variant.stock : 0;
            if (stock <= 0) {
                return { success: false, message: 'Cannot add items. One or more products in your wishlist are out of stock.' };
            }
        }
        ...
        for (const item of activeWishlistItems) {
            ...
            const result = await addToCartService(userId, item.productId._id, variantIdToUse, 1);
            if (result.success) {
                item.isActive = false;
                addedCount++;
            } else {
        ...
        if (addedCount > 0) {
            ...
            return { 
                success: false, 
                message: errors.length > 0 ? errors[0] : 'No items could be added to cart.' 
            };
        }
    } catch (error) {
