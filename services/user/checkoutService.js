...
const getCheckoutAddressDataService = async (userId) => {
  ...
  const defaultAddress =
    (await addressModal.findOne({ userId, isDefault: true })) ||
    addresses[0] ||
    null;
  ...
  if (productId) {
    ...
    for (const item of cart.items) {
      ...
      if (item.isUnavailable) {
        const err = new Error(
          `"${item.name}" is currently unavailable. Please remove it from your cart before proceeding.`,
        );
        err.statusCode = 400;
      ...
      if (item.stock <= 0) {
        const err = new Error(
          `"${item.name}" is out of stock. Please remove it from your cart before proceeding.`,
        );
        err.statusCode = 400;
      ...
      if (item.quantity > item.stock) {
        const err = new Error(
          `Not enough stock for "${item.name}". Only ${item.stock} unit(s) available but ${item.quantity} in cart.`,
        );
        err.statusCode = 400;
...
const getCheckoutPageDataService = async (userId, query) => {
  const { addresses, defaultAddress } =
    await getCheckoutAddressDataService(userId);
...
const placeOrderService = async (userId, query, body) => {
  ...
  for (const item of cart.items) {
    ...
    if (product.isHidden || product.status === false) {
      throw new Error(
        `"${item.name}" has been temporarily disabled and cannot be ordered right now.`,
      );
    }
    ...
    if (
      product.category &&
      (product.category.isHidden === true || product.category.status === false)
    ) {
      throw new Error(
        `"${item.name}" belongs to a category that is currently unavailable.`,
      );
    }
    ...
    if (item.variantId) {
      ...
      if (variant.stock < item.quantity) {
        throw new Error(
          `Insufficient stock for "${item.name}" (${item.variantName || "Default"}). Only ${variant.stock} left.`,
        );
      }
    }
  }
  ...
  if (couponId) {
    ...
    if (coupon.applicableCategories && coupon.applicableCategories.length > 0) {
      ...
      for (const item of cart.items) {
        ...
        const itemCategoryId =
          product && product.category ? product.category.toString() : null;

        if (
          !itemCategoryId ||
          !applicableCategoryIds.includes(itemCategoryId)
        ) {
      ...
      if (invalidItems.length > 0) {
        ...
        throw new Error(
          `This coupon cannot be used for the following products in your checkout: (${namesStr}). Please remove them to proceed with this coupon.`,
        );
      }
    }
    ...
    if (coupon.discountType === "percentage") {
      ...
      if (
        coupon.maxRedeemableAmount !== undefined &&
        coupon.maxRedeemableAmount !== null &&
        coupon.maxRedeemableAmount > 0
      ) {
  ...
  const finalGrandTotal = Math.max(
    0,
    cart.grandTotal - couponDiscount + shippingCost,
  );
