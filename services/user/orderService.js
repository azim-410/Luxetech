...
const cancelOrderService = async (orderId, userId, cancelItemIds, reason, comments) => {
    try {
        const order = await Order.findOne({ _id: orderId, userId });
        if (!order) {
        ...
        for (const item of order.items) {
            ...
            const shouldCancel = cancelItemIds.some(id => String(id) === String(item._id) || String(id) === String(item.productId));
...
const returnOrderService = async (orderId, userId, selectedItems, reason, comments, resolution, method) => {
    try {
        const order = await Order.findOne({ _id: orderId, userId });
        if (!order) {
            throw new Error('Order not found');
        ...
        // Validate order status
        if (order.orderStatus !== 'Delivered' && order.orderStatus !== 'Returned' && order.orderStatus !== 'Replaced') {
        ...
        const diffDays = (new Date().getTime() - new Date(refDate).getTime()) / (1000 * 60 * 60 * 24);
        if (diffDays > 7) {
        ...
        for (const item of order.items) {
            const shouldReturn = selectedItems.some(id => String(id) === String(item._id) || String(id) === String(item.productId));

            if (shouldReturn) {
                if (item.status === 'Cancelled') {
                    throw new Error(`Item "${item.name}" is already cancelled and cannot be returned`);
                }
                if (['Return Requested', 'Return Confirmed', 'Replacement Confirmed', 'Returned', 'Replaced'].includes(item.status)) {
                    throw new Error(`Item "${item.name}" has already been returned or has a return/replacement request`);
                }
        ...
        const allReturnedOrCancelled = order.items.every(item => 
            ['Return Requested', 'Return Confirmed', 'Replacement Confirmed', 'Returned', 'Replaced', 'Cancelled'].includes(item.status)
        );
