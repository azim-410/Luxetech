...
const updateProfileService = async (userId, name, email, profileImage) => {
  ...
  return {
    success: true,
    requiresOtp: true,
    message: "OTP sent to your current email. Please verify.",
  };
};
...
const verifyCurrentOtpService = async (userId, otp) => {
  ...
  return {
    success: true,
    message: "Current email verified. OTP sent to your new email.",
  };
};
...
const resendOtpService = async (userId) => {
  ...
  try {
    ...
    console.log(
      "OTP saved but email failed. User can continue with verification.",
    );
  }
...
const verifyOldPasswordService = async (userId, oldPassword) => {
  ...
  if (!EMAIL_REGEX.test(user.email)) {
    throw new Error(
      "Your account email appears to be invalid. Please contact support.",
    );
  }
...
const getProfileOtpTimerData = async (userId) => {
  ...
  if (!tempUser || !tempUser.emailOtpExpiry)
    return { remainingSeconds: 0, resendCooldownSeconds: 0, email };

  const remainingSeconds = Math.max(
    0,
    Math.floor(
      (new Date(tempUser.emailOtpExpiry).getTime() - Date.now()) / 1000,
    ),
  );
  const elapsedSinceCreated = Math.floor(
    (Date.now() - new Date(tempUser.createdAt).getTime()) / 1000,
  );
  const resendCooldownSeconds = Math.max(0, 60 - elapsedSinceCreated);
...
const getProfileDashboardData = async (userId) => {
  ...
  const latestAddress = addresses.find(
    (a) => !savedAddresses.some((sa) => sa._id.toString() === a._id.toString()),
  );
  if (latestAddress) {
